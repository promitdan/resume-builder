...
export default function CreativeTemplate({ content = {}, paletteColors = {}, pageIndex = 0 }) {
  ...
  const sectionLabel = (text) => (
    <div style={{ marginBottom: '14px' }}>
      <div style={{ fontSize: 'var(--resume-label)', fontWeight: '700', letterSpacing: '3px', textTransform: 'uppercase', color: c.headingText, marginBottom: '4px' }}>{text}</div>
                ...
                {education.map((e, i) => {
                  ...
                  return (
                    <div key={e.id ?? i} data-item={e.id ?? `edu-${i}`} style={{ paddingLeft: '14px', borderLeft: `3px solid ${bc}` }}>
                      ...
                      <div style={{ fontSize: 'var(--resume-body)', color: '#555' }}>
            ...
            <div key={key} data-section="languages" style={{ marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {languages.map((lang, i) => (
                  <span key={lang.id ?? i} style={{ background: '#f1f5f9', border: '1px solid #e2e8f0', color: '#64748b', fontSize: 'var(--resume-meta)', fontWeight: '400', padding: '5px 13px', borderRadius: '20px' }}>
              ...
              {projects.map((proj, i) => {
                ...
                return (
                  <div key={proj.id ?? i} data-item={proj.id ?? `proj-${i}`} style={{ marginBottom: l.itemSpacing, paddingLeft: '14px', borderLeft: `3px solid ${bc}` }}>
                    ...
                    {proj.description && (
                      <div style={{ fontSize: 'var(--resume-body)', lineHeight: '1.65', color: '#444', marginTop: '2px' }}>
                        <RichTextEditor path={`projects.${i}.description`} value={proj.description} />
                      </div>
                    )}
                    {proj.url && (
                      <div style={{ fontSize: 'var(--resume-meta)', color: '#999', marginTop: '2px' }}>
                ...
                return (
                  <div key={sec.id ?? i} style={{ marginBottom: '24px' }}>
                    ...
                    <InlineEditor path={`custom.${i}.description`} value={sec.description} multiline>
                      <div>
                        {lines.map((line, li) => (
                          <div key={li} style={{ fontSize: 'var(--resume-body)', lineHeight: '1.65', color: '#444', marginBottom: '2px' }}>• {line}</div>
                        ))}
                      </div>
                    </InlineEditor>
                  </div>
