                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                  {skills.map((sk, si) =>
                    (sk.items ?? []).map((item, ii) => (
                      <span key={`${si}-${ii}`} style={{ display: 'inline-block', background: '#f2f2f2', border: `1px solid ${c.dividerColor}`, borderRadius: '3px', padding: '4px 12px', fontSize: '13px', color: '#333' }}>
              ...
              {experience.map((e, i) => (
                <div key={e.id ?? i} style={{ marginBottom: l.itemSpacing }}>
                  ...
                  {e.bullets?.filter(Boolean).map((b, bi) => (
                    <div key={bi} style={{ display: 'flex', alignItems: 'baseline', fontSize: ty.bodyFontSize, lineHeight: '1.6', color: '#333', marginBottom: '3px' }}>
