          ...
          <div key={key}>
            ...
            {experience.map((e, i) => (
              <div key={e.id ?? i} style={{ marginBottom: l.itemSpacing }}>
                ...
                {dateStr(e) && <div style={{ fontSize: ty.bodyFontSize, color: c.dateMuted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.3px', margin: '2px 0 4px' }}>{dateStr(e)}</div>}
  ...
  return (
    <div style={{ fontFamily: ty.bodyFont, fontSize: ty.bodyFontSize, color: c.mainText, lineHeight: ty.bodyLineHeight, background: c.mainBg, minHeight: '11in' }}>
      ...
      <div style={{ background: c.bannerBg, padding: '28px 28px 22px 28px' }}>
        ...
        {personal.title && <div style={{ fontSize: '13px', color: c.bannerText, opacity: 0.75, marginTop: '5px' }}><InlineEditor path="personal.title" value={personal.title}>{personal.title}</InlineEditor></div>}
      </div>
