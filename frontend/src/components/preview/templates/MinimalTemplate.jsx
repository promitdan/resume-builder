...
export default function MinimalTemplate({ content = {}, paletteColors = {}, pageIndex = 0 }) {
  ...
  const sectionLabel = (text, first = false) => (
    <div style={{ fontSize: 'var(--resume-label)', fontWeight: '600', letterSpacing: '3px', textTransform: 'uppercase', color: '#aaa', marginBottom: '10px', marginTop: first ? '0' : '32px' }}>{text}</div>
  )
              ...
              <div style={{ display: 'flex', gap: '40px', flexWrap: 'wrap' }}>
                {education.map((e, i) => (
                  <div key={e.id ?? i} data-item={e.id ?? `edu-${i}`}>
                    ...
                    <div style={{ fontSize: 'var(--resume-body)', color: '#666' }}>
              ...
              {certifications.map((cert, i) => (
                <div key={cert.id ?? i} data-item={cert.id ?? `cert-${i}`} style={{ marginBottom: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <div>
                    ...
                    {cert.issuer && (
                      <span style={{ fontSize: 'var(--resume-body)', color: '#666' }}>
                        {' · '}<InlineEditor path={`certifications.${i}.issuer`} value={cert.issuer}>{cert.issuer}</InlineEditor>
                      </span>
                    )}
                  </div>
                  {cert.date && (
                    <span style={{ fontSize: 'var(--resume-meta)', color: '#999' }}>
            ...
            <div key={key} data-section="projects">
              ...
              {projects.map((proj, i) => (
                <div key={proj.id ?? i} data-item={proj.id ?? `proj-${i}`} style={{ marginBottom: l.itemSpacing }}>
                  ...
                  {proj.description && (
                    <div style={{ fontSize: 'var(--resume-body)', lineHeight: '1.7', color: '#444', marginTop: '2px' }}>
                ...
                return (
                  <div key={sec.id ?? i}>
                    ...
                    <InlineEditor path={`custom.${i}.description`} value={sec.description} multiline>
                      <div>
                        {lines.map((line, li) => (
                          <div key={li} style={{ fontSize: 'var(--resume-body)', lineHeight: '1.7', color: '#444' }}>• {line}</div>
                        ))}
                      </div>
                    </InlineEditor>
                  </div>
