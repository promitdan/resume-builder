...
export default function ModernSidebarTemplate({ content = {}, paletteColors = {} }) {
  ...
  const sidebar = (
    <div style={{ width: `${l.sidebarWidthPercent}%`, background: c.sidebarBg, color: c.sidebarText, padding: '32px 22px', boxSizing: 'border-box', flexShrink: 0, fontFamily: ty.bodyFont }}>
      {/* Name */}
      ...
      {(personal.email || personal.phone || personal.location) && (
        <>
          ...
          <div style={{ fontSize: ty.bodyFontSize, color: c.sidebarText }}>
            {personal.email && (
      ...
      {hasSkills && (
        <>
          {sidebarLabel('Skills')}
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, fontSize: ty.bodyFontSize, lineHeight: '1.8' }}>
  ...
  const main = (
    <div style={{ flex: 1, background: c.mainBg, color: c.mainText, padding: '32px 28px', boxSizing: 'border-box', fontFamily: ty.bodyFont }}>
      {personal.summary && (
        <>
          {mainLabel('Summary')}
      ...
      {sectionOrder.filter(k => !['personal', 'skills', 'languages'].includes(k)).map(key => {
          ...
          <div key={key}>
            ...
            {experience.map((e, i) => (
              <div key={e.id ?? i} style={{ marginBottom: l.itemSpacing }}>
                <div style={{ fontSize: '11px', color: c.mutedText, marginBottom: '2px' }}>{dateStr(e)}</div>
                ...
                {e.company && (
                  <div style={{ fontSize: ty.bodyFontSize, color: c.mutedText, marginBottom: '5px' }}>
        ...
        if (key === 'education' && education.length > 0) return (
          <div key={key}>
            {mainLabel('Education')}
            {education.map((e, i) => (
              <div key={e.id ?? i} style={{ marginBottom: '12px' }}>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '11in', fontFamily: ty.bodyFont, fontSize: ty.bodyFontSize, color: c.mainText, lineHeight: ty.bodyLineHeight, background: c.mainBg }}>
      {sidebar}
      {main}
    </div>
  )
