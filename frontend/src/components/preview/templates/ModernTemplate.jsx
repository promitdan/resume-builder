              ...
              {certifications.map((cert, i) => (
                <div key={cert.id ?? i} style={{ marginBottom: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <div>
                    ...
                    {cert.issuer && (
                      <span style={{ fontSize: 'var(--resume-meta)', color: '#555', fontStyle: 'italic' }}>
                        {' · '}<InlineEditor path={`certifications.${i}.issuer`} value={cert.issuer}>{cert.issuer}</InlineEditor>
                      </span>
                    )}
                  </div>
                  {cert.date && (
                    <span style={{ fontSize: 'var(--resume-meta)', color: '#888' }}>
            ...
            <div key={key}>
              ...
              {projects.map((proj, i) => (
                <div key={proj.id ?? i} style={{ marginBottom: l.itemSpacing }}>
                  ...
                  {proj.description && (
                    <div style={{ fontSize: 'var(--resume-body)', lineHeight: '1.65', color: '#333', marginTop: '2px' }}>
                ...
                return (
                  <div key={sec.id ?? i}>
                    ...
                    <InlineEditor path={`custom.${i}.description`} value={sec.description} multiline>
                      <div>
                        {lines.map((line, li) => (
                          <div key={li} style={{ fontSize: 'var(--resume-body)', lineHeight: '1.65', color: '#333', marginBottom: '2px' }}>• {line}</div>
                        ))}
                      </div>
                    </InlineEditor>
                  </div>
