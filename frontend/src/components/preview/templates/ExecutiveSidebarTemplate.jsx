...
export default function ExecutiveSidebarTemplate({ content = {}, paletteColors = {}, pageIndex = 0 }) {
  ...
  const mainLabel = (text) => <SectionHeader>{text}</SectionHeader>
  ...
  const renderSection = (key, col) => {
    ...
    const divider    = col === 'left' ? <div style={{ height: '1px', background: c.dividerColor, margin: '16px 0' }} /> : null
        ...
        <div key={key} data-section="experience">
          ...
          {experience.map((e, i) => (
            <div key={e.id ?? i} data-item={e.id ?? i} style={{ marginBottom: l.itemSpacing }}>
              ...
              {!e._bulletContinuation && dateStr(e) && <div style={{ fontSize: 'var(--resume-body)', color: mutedColor, marginBottom: '3px' }}>{dateStr(e)}</div>}
          ...
          {education.map((e, i) => (
            <div key={e.id ?? i} data-item={e.id ?? i} style={{ marginBottom: '10px', fontSize: 'var(--resume-body)' }}>
              <div style={{ fontWeight: 700, color: textColor }}>
                ...
                {e.location && <span style={{ fontWeight: 400, color: mutedColor }}>, <InlineEditor path={`education.${i}.location`} value={e.location}>{e.location}</InlineEditor></span>}
              </div>
              <div style={{ color: textColor }}>
                <InlineEditor path={`education.${i}.institution`} value={e.institution}>{e.institution}</InlineEditor>
              </div>
              {(e.startDate || e.endDate) && <div style={{ color: mutedColor }}>{[e.startDate, e.endDate].filter(Boolean).join(' — ')}{e.gpa ? ` · GPA: ${e.gpa}` : ''}</div>}
            </div>
          ))}
        </div>
      )
      ...
      return (
        <div key={key} data-section="certifications">
          ...
          {certifications.map((cert, i) => (
            <div key={cert.id ?? i} data-item={cert.id ?? i} style={{ marginBottom: '6px', fontSize: 'var(--resume-body)', color: textColor }}>
    ...
    if (key === 'awards') {
      ...
      return (
        <div key={key} data-section="awards">
          ...
          {label('Achievements')}
          {awards.map((aw, i) => (
            <div key={aw.id ?? i} data-item={aw.id ?? i} style={{ marginBottom: '6px', fontSize: 'var(--resume-body)', color: textColor }}>
        ...
        <div key={key} data-section="projects">
          ...
          {projects.map((proj, i) => (
            <div key={proj.id ?? i} data-item={proj.id ?? i} style={{ marginBottom: l.itemSpacing }}>
              ...
              {proj.description && <div style={{ fontSize: 'var(--resume-body)', lineHeight: ty.bodyLineHeight, marginTop: '2px', color: textColor }}><RichTextEditor path={`projects.${i}.description`} value={proj.description} /></div>}
            </div>
          ))}
        </div>
      )
        ...
        <div key={key} data-section="custom">
          ...
          {custom.map((sec, i) => (
            <div key={sec.id ?? i} data-item={sec.id ?? i}>
              ...
              <div style={{ fontSize: 'var(--resume-body)', lineHeight: ty.bodyLineHeight, color: textColor }}><RichTextEditor path={`custom.${i}.description`} value={sec.description} /></div>
            </div>
          ))}
        </div>
      )
