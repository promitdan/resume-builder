  ...
  return (
    <div style={{ fontFamily: ty.bodyFont, fontSize: ty.bodyFontSize, color: c.mainText, lineHeight: ty.bodyLineHeight, background: c.mainBackground }}>
      ...
      <div style={{ textAlign: 'center', padding: `28px ${SP} 12px` }}>
        ...
        {personal.location && (
          <div style={{ fontSize: '13px', color: c.mutedText, marginBottom: '3px' }}>
            <InlineEditor path="personal.location" value={personal.location}>{personal.location}</InlineEditor>
          </div>
        )}
        {personal.title && (
          <div style={{ fontSize: '14px', fontStyle: 'italic', color: c.mutedText, marginBottom: '4px' }}>
            <InlineEditor path="personal.title" value={personal.title}>{personal.title}</InlineEditor>
          </div>
        )}
        <div style={{ display: 'flex', justifyContent: 'center', gap: '28px', fontSize: '13px', color: c.mainText }}>
      ...
      {hasLinks && (
        <>
          ...
          <div style={{ padding: `0 ${SP}`, marginBottom: '4px', display: 'grid', gridTemplateColumns: 'max-content 1fr', gap: '2px 12px' }}>
            {personal.linkedin && (
              <><span style={{ fontWeight: 600, fontSize: ty.bodyFontSize }}>LinkedIn:</span>
              <ContactLink path="personal.linkedin" value={personal.linkedin} /></>
            )}
            {personal.website && (
              <><span style={{ fontWeight: 600, fontSize: ty.bodyFontSize }}>Website:</span>
      ...
      {personal.summary && (
        <>
          {band('Summary')}
          <div style={{ padding: `0 ${SP}`, fontSize: ty.bodyFontSize, lineHeight: ty.bodyLineHeight }}>
      ...
      {sectionOrder.filter(k => k !== 'personal').map(key => {
              ...
              {experience.map((e, i) => (
                <div key={e.id ?? i} style={{ marginBottom: l.itemSpacing }}>
                  <div style={{ display: 'flex', alignItems: 'baseline' }}>
                    ...
                    <span style={{ whiteSpace: 'nowrap', fontSize: '13px', color: c.mutedText }}>{dateStr(e)}</span>
                  </div>
                  {e.bullets?.filter(Boolean).map((b, bi) => (
                    <div key={bi} style={{ paddingLeft: '18px', fontSize: ty.bodyFontSize, lineHeight: ty.bodyLineHeight, color: c.mainText, marginTop: '3px' }}>
              ...
              {education.map((e, i) => (
                <div key={e.id ?? i} style={{ marginBottom: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'baseline' }}>
                    ...
                    <span style={{ flex: 1, borderBottom: '1px dotted #aaa', margin: '0 8px 3px', minWidth: '16px' }} />
                    <span style={{ whiteSpace: 'nowrap', fontSize: '13px', color: c.mutedText }}>
                      {[e.startDate, e.endDate].filter(Boolean).join(' — ')}
                    </span>
                  </div>
                  <div style={{ paddingLeft: '18px', fontStyle: 'italic', color: c.mutedText, fontSize: ty.bodyFontSize }}>
                ...
                {skills.map((sk, si) =>
                  (sk.items ?? []).map((item, ii) => (
                    <div key={`${si}-${ii}`} style={{ display: 'flex', alignItems: 'baseline', paddingRight: '12px' }}>
                      ...
                      <span style={{ whiteSpace: 'nowrap', fontSize: ty.bodyFontSize }}>
              ...
              {certifications.map((cert, i) => (
                <div key={cert.id ?? i} style={{ marginBottom: '8px' }}>
                  <DotLeader
                    ...
                    rightStyle={{ color: c.mutedText, fontSize: '13px' }}
                  />
                </div>
              ))}
            </div>
        ...
        if (key === 'languages' && languages.length > 0) return (
          <div key={key}>
            {band('Languages')}
            <div style={{ padding: `0 ${SP}`, display: 'flex', flexWrap: 'wrap', gap: '4px 32px', fontSize: ty.bodyFontSize }}>
              ...
              {projects.map((proj, i) => (
                <div key={proj.id ?? i} style={{ marginBottom: l.itemSpacing }}>
                  <div style={{ display: 'flex', alignItems: 'baseline' }}>
                    ...
                    <ContactLink path={`projects.${i}.url`} value={proj.url} style={{ fontSize: '13px', color: c.mutedText }} /></>}
                  </div>
                  {proj.description && (
                    <div style={{ paddingLeft: '18px', fontSize: ty.bodyFontSize, lineHeight: ty.bodyLineHeight, marginTop: '3px' }}>
